...
bool diagonal(const char* word, char matrix[][23], int wordLen) {
    for (int i = 0; i < 23; i++) {
        for (int j = 0; j < 23; ++j) {
            bool found = true;
            
            if (i <= 23 - wordLen && j <= 23 - wordLen) {
                for (int k = 0; k < wordLen; k++) {
...
bool searchWord(const char* word, char matrix[][23]) {
